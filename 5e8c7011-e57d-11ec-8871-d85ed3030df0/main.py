...
def frash_first_stock():
    url = 'https://api.heheapp.com/api/users_spool_weicai?name=pool-1&' + urlencode({"query":"10个交易日的区间平均成交额>4亿元，1天前未张停，2天前未涨停，非创业板，上个交易日ma3大于ma10，前10日涨幅小于50，竞价涨幅小于7%，价格小于70"})
...
def on_tick(context, tick):
    ...
    #回封下单
    elif tick.open == upper_limit and tick.low != upper_limit and tick.quotes[0]['ask_p'] == 0 and tick.quotes[0]['bid_v'] * tick.quotes[0]['bid_p'] > context.retery and deal <= context.deal_time and tick.symbol not in context.unfinished_orders:
