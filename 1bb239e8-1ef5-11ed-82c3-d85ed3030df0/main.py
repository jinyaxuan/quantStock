...
def frash_first_stock(context):
    url = 'https://api.heheapp.com/api/users_spool_weicai?name=pool-1&' + urlencode(
        {"query": "上个交易日到前20个交易日的区间平均成交额>3亿元，上个交易日未张停，上个交易日曾涨停取反，上上个交易日未涨停，非创业板，近20日涨幅小于30，竞价涨幅小于7%，可转债取反，主板，1%<上个交易日实体≤5% ，上个交易日振幅小于9%，今日涨停价>上个交易日90%成本上限，剔除上个交易日竞价抢筹，10日内的涨跌幅<0%出现次数>=3"})
...
def frash_black_stock():
    ...
    url = 'https://api.heheapp.com/api/users_black_list_weicai?' + \
        urlencode({"query": "10个交易日的区间日均换手率>30%||上市天数>30日取反||煤炭||证券||银行"})
    res = requests.put(headers=headers, url=url)
    res.enconding = "utf-8"
...
def algo_1(context):
    ...
    for Account_position in Account_positions:
        ...
        context.sell[Account_position['symbol']] = [0, 0]
    ...
    context.instrument = get_instruments(symbols=list(
        context.stocks.keys()), df=True, fields='symbol, lower_limit')
    ...
    log(level='info', msg='目前持仓:{}'.format(
        list(context.stocks.keys())), source='strategy')
...
def on_tick(context, tick):
    ...
    close = context.history_data[context.history_data['symbol']
                                 == tick.symbol].close.values[0]
    low_limit = context.instrument[context.instrument['symbol']
                                   == tick.symbol].lower_limit.values[0]
    ...
    if pos and pos.available_now > 0 and tick.low != 0 and context.still == 1 and context.stocks[tick.symbol] > 0:
        ...
        if pos.price < tick.open * 0.98 and pos.price < low_limit * 1.005 and context.nuclear_button == 1:
            ...
            log(level='info', msg='{}, 止损'.format(
                tick.symbol), source='strategy')

        elif tick.quotes[0]['bid_v'] * tick.quotes[0]['bid_p'] < 10000000 and tick.quotes[0]['ask_p'] == 0:

            order_volume(symbol=tick.symbol, volume=pos.available_now, side=OrderSide_Sell, order_type=OrderType_Limit,
                         position_effect=PositionEffect_Close, price=low_limit)
            log(level='info', msg='{}, 炸板了'.format(
                tick.symbol), source='strategy')
...
def on_parameter(context, parameter):
    if parameter['name'] == '核按钮' and parameter['value'] != context.nuclear_button:
        ...
        set_parameter(key='nuclear_button', value=context.nuclear_button, min=0, max=1, name='核按钮', intro='核按钮开关',
                      group='打板', readonly=False)
        log(level='info', msg='核按钮:{}'.format(
            nuclear[context.nuclear_button]), source='strategy')
        ...
        set_parameter(key='still', value=context.still, min=0, max=1, name='交易开关', intro='交易开关',
                      group='打板', readonly=False)
        log(level='info', msg='交易开关:{}'.format(
            nuclear[context.still]), source='strategy')
        ...
        if parameter['value'] != context.stocks[parameter['name']]:
            ...
            log(level='info', msg='{}的卖出仓位为:{}'.format(
                parameter['name'], context.stocks[parameter['name']]), source='strategy')
