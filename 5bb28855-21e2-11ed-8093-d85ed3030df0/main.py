...
def frash_first_stock(context):
    url = 'https://api.heheapp.com/api/users_spool_weicai?name=pool-1&' + urlencode(
        {"query": "上个交易日到前20个交易日的区间平均成交额>3亿元，上个交易日未张停，上个交易日曾涨停取反，上上个交易日未涨停，非创业板，上个交易日ma3大于ma10，近20日涨幅小于35，竞价涨幅小于7%，可转债取反，主板，1%<上个交易日实体≤5% ，上个交易日ma10大于ma20，上个交易日振幅小于9%，今日涨停价>上个交易日90%成本上限，剔除上个交易日竞价抢筹，10日内的涨跌幅<0%出现次数>=3"})
...
def on_tick(context, tick):
    ...
    if pos and pos.available_now > 0 and tick.low != 0 and context.still == 1 and context.stocks[tick.symbol] > 0: 
        ...
        if pos.price / avg >= 1.025 and context.sell[tick.symbol][0] == 0 and context.stocks[tick.symbol] > 0:
            ...
            order_volume(symbol=tick.symbol, volume=math.ceil(pos.available_now // 100 // 4) * 100 , side=OrderSide_Sell,
        ...
        if pos.price / tick.low >= 1.035 and context.sell[tick.symbol][0] == 0 and context.stocks[tick.symbol] > 0:
        ...
        if pos.price / avg >= 1.035 and context.sell[tick.symbol][1] == 0 and context.stocks[tick.symbol] > 0:

            order_volume(symbol=tick.symbol, volume=math.ceil(pos.available_now // 100 // 2) * 100, side=OrderSide_Sell,
                            order_type=OrderType_Limit, position_effect=PositionEffect_Close, price=low_limit)
        ...
        if pos.price / tick.low >= 1.055 and context.sell[tick.symbol][1] == 0 and context.stocks[tick.symbol] > 0:
        ...
        if pos.price / avg >= 1.045 and context.sell[tick.symbol][2] == 0 and context.stocks[tick.symbol] > 0:

            order_volume(symbol=tick.symbol, volume=math.ceil(pos.available_now // 100) * 100, side=OrderSide_Sell,
                            order_type=OrderType_Limit, position_effect=PositionEffect_Close, price=low_limit)
        ...
        if pos.price / tick.low >= 1.075 and context.sell[tick.symbol][2] == 0 and context.stocks[tick.symbol] > 0:
        ...
        if tick.quotes[0]['bid_v'] * tick.quotes[0]['bid_p'] < 10000000 and tick.quotes[0]['ask_p'] == 0:
        ...
        if pos.price / tick.high <= 0.95 and context.stocks[tick.symbol] > 0:

            order_volume(symbol=tick.symbol, volume=math.ceil(pos.available_now // 100) * 100, side=OrderSide_Sell,
                            order_type=OrderType_Limit, position_effect=PositionEffect_Close, price=low_limit)

            log(level='info', msg='{}, 回落5%，清仓'.format(tick.symbol), source='strategy')

        # if tick.price / pos.vwap_diluted <= 0.95:
        #     order_volume(symbol=tick.symbol, volume=math.ceil(pos.available_now // 100) * 100, side=OrderSide_Sell,
        #                     order_type=OrderType_Limit, position_effect=PositionEffect_Close, price=low_limit)

        #     log(level='info', msg='{}, 小于成本价5%，清仓'.format(tick.symbol), source='strategy')
