...
headers ={
        ...
        "authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJOVFUyT1E9PSIsImV4cCI6MTY2MjI5MjYxNH0.v-GXvSG9BDlObifRbZvvyeGL9_wSs4C1Q2Z9YnuRIdI",
...
def Get_stock(black_stock):  
    stocks = []
    url = 'https://api.heheapp.com/api/users_spool?name=pool-0.'
...
def init(context):
    ...
    schedule(schedule_func=algo_1, date_rule='1d', time_rule='9:29:55')
...
def algo_1(context):
    ...
    # Account_positions = [i['symbol'] for i in context.account().positions()]
    ...
    for stock in context.stocks:
        ...
        if stock not in context.stocks_deal.keys():
            context.stocks_deal[stock] = [0, 0, 0]
        # if stock in Account_positions:
        #     context.stocks_deal[stock] = [1, 1, 1]
...
def on_tick(context, tick):
    ...
    if amount / context.cash_nav < 0.35:
        ...
        if tick.open > context.stocks_close[tick.symbol][-2] * 0.99 and tick.high > context.stocks_close[tick.symbol][-2] * 1.02 and tick.price < tick.high * 0.95 and context.stocks_deal[tick.symbol][0] == 0:
            order_percent(symbol=tick.symbol, percent=0.1, side=OrderSide_Buy, order_type=OrderType_Limit, position_effect=PositionEffect_Open, price=upper_limit)
            log(level='info', msg='{}, 回落5%下单'.format(tick.symbol), source='strategy')
        ...
        if tick.price <= MA_4 and context.stocks_deal[tick.symbol][1] == 0 and tick.high > MA_4:
            ...
            log(level='info', msg='{}, 跌破MA_4'.format(tick.symbol), source='strategy')
        ...
        if tick.price <= MA_9 and context.stocks_deal[tick.symbol][2] == 0 and tick.high > MA_9:
            ...
            log(level='info', msg='{}, 跌破MA_9'.format(tick.symbol), source='strategy')
            context.stocks_deal[tick.symbol][2] = 1

    if context.stocks_deal[tick.symbol][0] == 1 and context.stocks_deal[tick.symbol][1] == 1 and context.stocks_deal[tick.symbol][2] == 1:
